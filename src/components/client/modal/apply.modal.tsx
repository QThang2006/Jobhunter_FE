...
const ApplyModal = (props: IProps) => {
    ...
    const handleOkButton = async () => {
        if (jobDetail?.endDate && dayjs(jobDetail.endDate).isBefore(dayjs(), 'day')) {
            notification.error({
                message: "Hết hạn nộp hồ sơ",
                description: "Job này đã hết hạn nộp hồ sơ, vui lòng chọn job khác!"
            });
            setIsModalOpen(false);
            return;
        }

        if (jobDetail?.startDate && dayjs(jobDetail.startDate).isAfter(dayjs(), 'day')) {
            notification.error({
                message: "Chưa đến thời gian nộp hồ sơ",
                description: "Job này chưa mở tuyển, vui lòng quay lại sau!"
            });
            setIsModalOpen(false);
            return;
        }
                                ...
                                <Row gutter={[10, 10]}>
                                    ...
                                    <Col span={24}>
                                        ...
                                        >
                                            ...
                                            <Upload {...propsUpload}>
                                                <Button icon={<UploadOutlined />}>Tải lên CV (*.pdf, *.doc, *.docx &lt; 5MB)</Button>
                                            </Upload>
                                        </ProForm.Item>
                                    </Col>
                                </Row>
